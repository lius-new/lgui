...
use lgui::core::{ellipse, UiEventContext};
...
pub fn render(rect: UiRect, state: State<AppState>) -> Element {
    ...
    let proj_btn = panel(proj, VisualStyle::default())
        ...
        .child(text(
            UiRect::new(proj.left + 8.0, rect.top + 10.0, proj.right, rect.top + 30.0),
            "▤ Project",
    ...
    bar = bar.child(text(
        UiRect::new(pill.left + 10.0, rect.top + 11.0, pill.right, rect.top + 29.0),
        "⑂ main*  +24 -3",
    ...
    let trig_btn = panel(trig, VisualStyle::filled(theme::SURFACE).radius(8.0))
        ...
        .child(text(
            UiRect::new(trig.left + 12.0, rect.top + 10.0, trig.left + 220.0, rect.top + 30.0),
            "⌕  src > Services > WalletService.cs",
            theme::mono(theme::ZINC_400, theme::SMALL),
        ))
        .child(text(
            UiRect::new(trig.right - 40.0, rect.top + 11.0, trig.right - 10.0, rect.top + 29.0),
            "⌘P",
    ...
    bar = bar.child(text(
        UiRect::new(tp.left + 10.0, rect.top + 11.0, tp.right, rect.top + 29.0),
        "▶ Test Run",
    ...
    let ab_btn = panel(ab, VisualStyle::default())
        ...
        .child(text(
            ...
            "✦",
            theme::sans(theme::ACCENT, theme::UI_SIZE),
        ));
    ...
    let min_btn = panel(min_r, VisualStyle::default())
        ...
        .child(text(
            UiRect::new(min_r.left, rect.top + 10.0, min_r.right, rect.top + 30.0),
            "─",
            theme::mono(theme::ZINC_400, theme::SMALL),
        ));
    ...
    let max_btn = panel(max_r, VisualStyle::default())
        ...
        .child(text(
            UiRect::new(max_r.left, rect.top + 12.0, max_r.right, rect.top + 30.0),
            "□",
            theme::mono(theme::ZINC_400, theme::SMALL),
        ));
    ...
    let close_btn = panel(close_r, VisualStyle::default())
        ...
        .child(text(
            UiRect::new(close_r.left, rect.top + 12.0, close_r.right, rect.top + 30.0),
            "✕",
            theme::mono(theme::ZINC_400, theme::SMALL),
        ));
