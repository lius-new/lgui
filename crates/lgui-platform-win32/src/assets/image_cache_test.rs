...
fn decoded_image_hit_does_not_recreate_the_image() {
    ...
    let _gdiplus =
        crate::gdiplus::GdiPlusRuntime::start().expect("start GDI+ for image cache test");
...
fn zero_budget_win32_cache_still_delivers_a_reachable_image() {
    ...
    let _gdiplus =
        crate::gdiplus::GdiPlusRuntime::start().expect("start GDI+ for image cache test");
    let mut options = lgui_core::memory::test_memory_options();
    options.budget.max_parallel_large_tasks = 1;
    let governor = lgui_core::memory::MemoryGovernor::new(options);
...
fn images_waiting_for_a_task_slot_are_loaded_in_order() {
    ...
    let mut options = lgui_core::memory::test_memory_options();
    options.budget.max_parallel_large_tasks = 1;
    let _memory = install_image_memory_governor(lgui_core::memory::MemoryGovernor::new(options));
    trim_cached_image_cache(0);
    let cache = portable_image_cache_handle();
