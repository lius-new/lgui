...
pub(super) fn create_window(
    ...
    let memory_domain = if renderer_name == "d2d" {
        lgui_core::memory::CacheDomain::D2d
    } else {
        lgui_core::memory::CacheDomain::Gdi
    };
    ...
    if let Some(executor) = lgui_core::backend::task_spawner(&context) {
        session.set_task_spawner(executor);
    }
    let (component_usage, host_scene_usage) = lgui_core::backend::session_memory_usage(&session);
    ...
    let component_memory_registration = {
        ...
        let trim = CoalescedTrim::new(dispatcher.clone(), move |target_bytes| {
            STATE.with(|state| {
                ...
                let released = lgui_core::backend::session_trim_component_outputs(
                    &mut window.session,
                    target_bytes,
                );
    ...
    let host_scene_memory_registration = {
        ...
        let trim = CoalescedTrim::new(dispatcher.clone(), move |_target_bytes| {
            STATE.with(|state| {
                ...
                let released = lgui_core::backend::session_trim_host_scene(&mut window.session);
                update_session_memory_usage(window);
                released
            })
        });
...
pub(super) fn suspend_window_rendering(hwnd: HWND, force: bool) -> bool {
    let memory = STATE.with(|state| {
        ...
        *window
            .renderer_memory
            .lock()
            .expect("renderer memory usage poisoned") = lgui_core::memory::CacheUsage::default();
    ...
    memory.notify(lgui_core::memory::MemoryEvent::WindowHidden);
    true
}

pub(super) fn update_session_memory_usage(window: &WindowState) {
    let (component, host_scene) = lgui_core::backend::session_memory_usage(&window.session);
    *window
        .component_memory
        .lock()
...
pub(super) fn suspend_application_if_backgrounded() {
    ...
    if let Some(context) = context {
        context
            .memory()
            .notify(lgui_core::memory::MemoryEvent::AllWindowsHidden);
    }
...
pub(super) fn custom_frame_hit_test(hwnd: HWND, lparam: LPARAM) -> Option<LRESULT> {
    ...
    STATE.with(|windows| {
        ...
        if let Some(hit) = state.session.tree().hit_test(logical_point) {
            return Some(LRESULT(
                if hit.interaction == lgui_core::core::InteractionRole::WindowDragRegion {
                    HTCAPTION as isize
                } else {
                    HTCLIENT as isize
                },
