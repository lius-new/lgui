...
pub(super) extern "system" fn window_proc(
    ...
    match message {
        ...
        crate::WM_IMAGE_CACHE_INVALIDATED => {
            let invalidated = crate::take_image_cache_invalidations();
            ...
            let focus = STATE.with(|state| {
                state.borrow().get(&(hwnd.0 as isize)).map(|window| {
                    (
                        ...
                        lgui_core::window::WindowFocusChanged {
        ...
        WM_DESTROY => {
            ...
            #[cfg(feature = "images-win32")]
            crate::clear_image_repaint_hwnd(hwnd);
            ...
            if let Some(dispatcher) = dispatcher {
                ...
                if let Some(raw) = next_window {
                    ...
                    #[cfg(feature = "images-win32")]
                    crate::register_image_repaint_hwnd(HWND(raw as _));
                } else {
    ...
    match policy {
        ...
        ClosePolicy::Notify => {
            if let Some(handler) = handler {
                let mut event = lgui_core::core::UiEventContext::new(context, id);
                handler(&mut event);
