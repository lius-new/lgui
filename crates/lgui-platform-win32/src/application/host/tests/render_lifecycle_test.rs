use super::super::*;
use lgui_core::core::{group, Scene};
use lgui_render_api::{RenderStats, RendererCapabilities};
use windows::Win32::UI::WindowsAndMessaging::{
    GetWindow, IsWindow, GW_HWNDNEXT, GW_OWNER, HWND_TOP, SWP_NOMOVE, SWP_NOSIZE, SW_MINIMIZE,
    SW_RESTORE, SW_SHOWMINNOACTIVE, WS_OVERLAPPEDWINDOW,
};
...
impl TestWindow {
    fn new(renderer: RecordingRenderer, builds: Arc<AtomicUsize>) -> Self {
        ...
        let hwnd = create_window(
            ...
            ApplicationContext::empty(lgui_core::memory::MemoryOptions::unbounded(
                lgui_core::memory::ImageCachePolicy::NoStore,
                false,
            )),
            Arc::new(renderer),
            Win32Dispatcher::new(),
...
fn owner_drag_leaves_chat_position_visibility_and_z_order_unchanged() {
    ...
    for _ in 0..2 {
        ...
        assert!(
            is_above(owner.hwnd, chat),
            "owner must remain above chat while unrelated windows may enter the Z-order"
        );
