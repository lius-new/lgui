...
impl SceneRenderer for D2dRenderer {
    ...
    fn trim(&mut self, pressure: lgui_render_api::MemoryPressure) {
        if let Some(resources) = self.resources.as_mut() {
            resources.renderer.trim_to(match pressure {
                lgui_render_api::MemoryPressure::Moderate => {
                    resources.renderer.memory_usage().cache_bytes / 2
                }
                lgui_render_api::MemoryPressure::Critical => 0,
            });
        }
    }
    ...
    fn memory_usage(&self) -> lgui_core::memory::CacheUsage {
        let Some(resources) = self.resources.as_ref() else {
            return lgui_core::memory::CacheUsage::default();
        };
