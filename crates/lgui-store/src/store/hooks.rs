...
use crate::StoreApplicationExt as _;
    ...
    T: Send + Sync + 'static,
{
    ...
    {
        let runtime = store_runtime(cx);
        use_definition(cx, runtime, self, selector)
    }
...
impl StoreHooks for RenderCx<'_, '_> {
    ...
    where
        ...
        F: Fn(&T) -> S + Send + Sync + 'static,
    {
        let runtime = store_runtime(self);
        use_unit(self, runtime, selector)
    }
}

fn store_runtime(cx: &mut RenderCx<'_, '_>) -> Arc<StoreRuntime> {
    cx.try_use_context::<StoreContext>()
        .map(|context| context.runtime)
        .unwrap_or_else(|| cx.application().stores())
}
