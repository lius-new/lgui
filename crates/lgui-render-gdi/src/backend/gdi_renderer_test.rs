...
fn gdi_shadow_uses_subtree_alpha_and_refreshes_cached_pixels() {
    let _gdiplus = lgui_platform_win32::GdiPlusRuntime::start().unwrap();
    ...
    output.redraw(shadow_scene(128, 2.0).commands(), &bounds);
    assert!(surface_pixel(&output.output, 44, 9)[3] > 0);
    output.redraw(shadow_scene(0, 2.0).commands(), &bounds);
    ...
    for (name, scene) in shape_shadow_scenes() {
        ...
        assert_shape_shadow("gdi", name, pixels);
    }
    drop(output);
