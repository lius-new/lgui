...
pub struct SkiaSoftwareSurface {
    pixels: Vec<u8>,
    size: (i32, i32),
    cache: SkiaCache,
}
...
impl SkiaSoftwareSurface {
    ...
    pub fn draw(&mut self, scene: &Scene, frame: &FrameInfo<'_>) -> Result<(), String> {
    ...
    pub fn cache_stats(&self) -> SkiaCacheStats {
        self.cache.stats()
    }
}

pub fn paint_scene_damage(
    canvas: &Canvas,
    cache: &mut SkiaCache,
    scene: &Scene,
    frame: &FrameInfo<'_>,
