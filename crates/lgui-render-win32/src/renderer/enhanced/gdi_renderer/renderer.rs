...
pub(super) fn draw_gdi_compositing_layer(
        ...
        || layer.shadow != lgui_core::backend::compositing_shadow(spec)
    {
        ...
        if let Some(shadow) = lgui_core::backend::compositing_shadow(spec) {
            let pixels =
                unsafe { std::slice::from_raw_parts_mut(layer.output.bits, layer.output.bytes) };
            lgui_core::backend::composite_shadow(pixels, width as usize, height as usize, shadow);
