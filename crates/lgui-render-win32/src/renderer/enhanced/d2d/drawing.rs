...
pub(super) fn draw_command_d2d(
    ...
    match command {
        ...
        ScenePrimitive::Glow {
            ...
            let style = OverlayStyle::new().radial(lgui_core::core::RadialGradientLayer::new(
                *color,
                *alpha as f32 / 255.0,
                0.5,
                0.5,
...
pub(super) fn draw_icon(
    ...
    let bitmap = if let Some(bitmap) = resources.bitmap_cache.get(&cache_key) {
        ...
        let Some(icon) = lgui_platform_win32::rasterize_svg_icon_bgra(key, rect, style) else {
            return Ok(());
        };
        let bitmap = create_bgra_bitmap(
            &resources.context,
...
pub(super) fn translate_command(command: &ScenePrimitive, dx: f32, dy: f32) -> ScenePrimitive {
    lgui_core::core::translate_scene_primitive_for_backend(command, dx, dy)
}
