...
> 临时权威文档。用于约束本轮内存治理重构的设计、阶段、迁移和验收。
> 实现完成后，长期有效的规则应合并到 `ARCHITECTURE.md`、`API.md` 和
> `BASELINE.md`，届时可以删除本文档。
...
| ID | 临时内容 | 引入阶段 | 删除条件 | 最迟删除阶段 |
| --- | --- | --- | --- | --- |
| MG-001 | 旧 `clear_image_caches`/`clear_render_caches` Facade | 1 | 所有内部调用迁移到 Registry；公开兼容策略确定 | 9 |
| MG-002 | 旧分散缓存向 Registry 报告但仍自行定预算 | 1 | Governor 成为唯一预算分配者 | 3 |
| MG-003 | `MemoryAndDisk` 暂时按 `Memory` 执行 | 3 | 真实 Persistent Store 与调用方迁移完成 | 6 |
| MG-004 | Thread-local/native Cache Adapter | 1 | Application/UiSession 生命周期和所有者线程回收完成 | 7 |
| MG-005 | 旧 `UiImageSource` 隐式 Auto Policy | 4 | 所有构造器已明确默认并完成兼容迁移 | 9 |

每个临时路径必须在代码中使用：

```rust,ignore
// COMPAT(MG-xxx): temporary memory-governance migration path.
// REMOVE WHEN: <可验证条件>.
```

不得新增未登记的 `COMPAT(MG-...)`，不得延长最迟删除阶段而不先修改本文档。
